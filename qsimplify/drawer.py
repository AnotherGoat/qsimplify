"""Contains functions capable of drawing quantum graphs and circuits."""
...
def save_circuit_png(circuit: QuantumCircuit, file_name: str) -> None:
...
def save_graph_png(graph: QuantumGraph, file_name: str) -> None:
    """Save a quantum graph to a png file."""
    logger.info("Saving graph to file {}.png", file_name)
    _save_graph(graph, file_name, "png", dpi=str(150))


def save_graph_svg(graph: QuantumGraph, file_name: str) -> None:
    """Save a quantum graph to a svg file."""
    logger.info("Saving graph to file {}.svg", file_name)
    _save_graph(graph, file_name, "svg")


def save_graph_to_buffer(graph: QuantumGraph, extension: str, dpi: int = 96) -> BytesIO:
    ...
    _draw_nodes(graph, image)
    _draw_edges(graph, image)
...
def _save_graph(graph: QuantumGraph, file_name: str, extension: str, **kwargs: str) -> None:
    image = graphviz.Digraph(format=extension)
...
def _draw_nodes(graph: QuantumGraph, image: Digraph) -> None:
    for node in graph:
        x, y = _find_draw_position(graph, node)

        settings = {
            "label": _find_node_label(node),
            "fillcolor": _find_node_color(node),
            "style": "filled",
            "shape": "circle",
            "width": str(1.5),
            "pos": f"{x},{y}!",
...
def _draw_edges(graph: QuantumGraph, image: Digraph) -> None:
    for edge in graph.iter_edges():
        ...
        settings = {
            ...
            "fontcolor": _find_edge_color(edge),
            "labeldistance": str(2.0),
            "style": "dashed",
            "color": _find_edge_color(edge),
        }
